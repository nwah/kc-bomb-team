...
#
#   make            build dist/BOMBTEAM.KCC   (KC 85/3 + 4, size-checked)
#   make kc85       the same, explicitly
#   make z9001      build dist/bombteam-z9001  (raw binary + .map; size-checked)
#   make z9001-tap  build dist/z9001.tap
...
KC_HDRS := src/hw.h src/view.h common/game.h common/bombs.h common/scores.h common/version.h src/udg.h
TARGET  := bombteam
...
Z9_HDRS := z9001/hw.h z9001/view.h common/game.h common/bombs.h common/scores.h common/version.h
Z9_TARGET := bombteam-z9001

all: kc85

# KC 85: loads at $0200.  The stack sits below the load address, so the
# binary need only end before $4000, the top of the /3's 16K.
dist/$(TARGET): $(KC_SRC) $(KC_HDRS)
	@mkdir -p dist
	$(ZCC) +kc $(HOMEINC) -o dist/$(TARGET) -create-app -m $(KC_SRC)

kc85: dist/$(TARGET) check-kc85

check-kc85: dist/$(TARGET) dist/$(TARGET).map
	./tools/check-size.sh dist/$(TARGET) 0200 0
...
.PHONY: all kc85 check-kc85 z9001 check-z9001 z9001-tap run z9001-run clean
